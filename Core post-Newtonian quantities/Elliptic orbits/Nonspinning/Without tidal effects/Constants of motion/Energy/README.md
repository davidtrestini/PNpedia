...
We also introduce the enhancement function $\Lambda(e) = \frac{1}{16}\sum_{p=1}^{\infty} p^2 \ln(p/2)$, where $e\in [0,1)$ and we use the Einstein summation on the spatial indices $i,j \in \{x,y,z\}$. The Fourier coefficients are explicitly expressed in terms of the Bessel functions $J_p(x)$ as follows:
